    ...
    const wedding = {
        ...
        section1(){
           ...
           function mainNextSlide(){
                ...
                $slide.eq(cnt).css({zIndex: 3}).stop().animate({opacity:0}, 0).animate({opacity:1}, 300); // 다음슬라이드
                pageBtn();
           }
           ...
           function mainPrevSlide(){
                ...
                $slide.eq(imsi!==null?imsi:(cnt===2?0:cnt+1)).css({zIndex: 3}).stop().animate({opacity:1}, 0).animate({opacity:0}, 300); // 현재슬라이드
                pageBtn()
           }
